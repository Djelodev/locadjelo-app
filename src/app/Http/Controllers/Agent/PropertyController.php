...
class PropertyController extends Controller
{
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'floor_plan'         => 'mimetypes:image/jpeg,image/png,image/gif,image/webp',
        ...
        if ($image) {
            ...
            $propertyimage = Image::make($image)->stream(); // FIX: was ->encode($ext, 90)
        ...
        $property->featured           = isset($request->featured) ? true : false;
        ...
        if ($gallary) {
            foreach ($gallary as $images) {
                ...
                $galimage['size']        = $images->getSize(); // FIX: was getClientSize()
                ...
                $propertyimage = Image::make($images)->stream(); // FIX: was ->encode($ext, 90)
                Storage::disk('public')->put('property/gallery/' . $galimage['name'], $propertyimage);
                $property->gallery()->create($galimage);
            }
        }

        Toastr::success('message', 'Propriété créée avec succès.');
    ...
    public function update(Request $request, $property)
    {
        ...
        Toastr::success('message', 'Propriété mise à jour avec succès.');
        return redirect()->route('agent.properties.index');
    }
    ...
    public function destroy(Property $property)
    {
        ...
        Toastr::success('message', 'Propriété supprimée avec succès.');
        return back();
    }

    public function galleryImageDelete(Request $request)
    {
        PropertyImageGallery::find($request->id)->delete();
